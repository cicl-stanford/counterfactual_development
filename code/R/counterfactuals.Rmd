...
###### Hypothesis 1 
```{r}
# Overall children will thank the individual who prevented the worse outcome 
# from occurring at rates that exceed chance
...
Adult performance was at 100%, so this model cannot be fitted. 

```{r eval=FALSE}

# Overall adults will thank the individual who prevented the worse outcome from occurring 
# at rates that exceed chance
...
df.exp2.age.min = as_tibble(t(preds)) %>%
  ...
  pivot_longer(-age, names_to = "draw", values_to = "p") %>%
  group_by(age) %>%
  summarise(
    mean = mean(p),
    lower = quantile(p, 0.025),
...
#### Per trial
```{r fig.width=33, fig.height=10, message=FALSE, warning=FALSE}
...
preds = posterior_epred(fit.brm5.children.age, newdata = df.exp3_age_grid, re_formula = NA)
...
ggplot(df.exp3.age.min, aes(x = age, y = mean)) +
  geom_ribbon(aes(ymin = lower, ymax = upper), 
              alpha = 0.2, fill = "darkgreen") +
  geom_line(color = "darkgreen", size = 1) +
  geom_hline(yintercept = 0.5, linetype = "dashed", color = "red") +
  geom_vline(xintercept = cross_age, linetype = "dotted", color = "blue") +
  annotate("text", x = cross_age, y = 0.52, 
           label = paste0("Crossing at age ≈ ", round(cross_age, 2)), 
...
# Posterior predictions
preds = posterior_epred(fit.brm7.children.age, newdata = df.exp3_age_grid, re_formula = NA)
...
```{r fig.width=43, fig.height=10, message=FALSE, warning=FALSE}
